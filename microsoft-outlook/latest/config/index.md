...
3. **Theme**: Choose the plugin theme.

   >**Note:** Outlook needs to be restarted to apply the new theme.
...
2. Configure the attributes that you need for the base `<outlook>` element:
    ...
    |`theme`|Sets the theme of the plugin|`classic`: Classic plugin theme. This is the default setting.<br><br>`dark`: Dark theme.|
...
3. Configure the attributes that you need for the `<storage>` element:
    ...
    |`storeFiles`|Controls the Extract email attachment archive option|`true`: email attachments from the Outlook inbox are extracted on upload to Alfresco. This is the default setting.<br><br>`false`: email attachments are not extracted on upload to Alfresco.|
    |`storeFilesFromDesktop`|Controls the Extract attachments from files archive option|`true`: email attachments from desktop files are extracted on upload to Alfresco.<br><br>`false`: email attachments are not extracted on upload to Alfresco. This is the default setting.|
